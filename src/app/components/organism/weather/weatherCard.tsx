...
const WeatherCard = ({ weather, location }: { weather: any, location: string }) => {
  ...
  return (
    <div className="relative flex flex-col my-6 bg-white shadow-sm border border-slate-200 rounded-lg w-[400px] h-[500px]">
      <div className="p-4 flex-1 overflow-y-auto">
        ...
        <p className="text-slate-600 mb-4 text-center">Water Temp: <span className="font-bold">{weather.water_temp}</span></p>
        {weather.days && weather.days.map((day: any) => (
          <div key={day.date} className="mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs border-collapse">
                ...
                <tbody>
                  {day.forecast.map((f: any, idx: number) => (
                    <tr
                      key={idx}
                      className={`transition hover:bg-blue-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}
                    >
                      <td className="py-1 px-2 text-center">{f.time}</td>
                      <td className="py-1 px-2 text-center">
                        <span className={`inline-block w-6 rounded-full ${f.rating === "3" ? "bg-green-400 text-white" : f.rating === "2" ? "bg-yellow-400 text-white" : f.rating === "1" ? "bg-red-400 text-white" : "bg-gray-200"}`}>
                          {f.rating || "-"}
                        </span>
                      </td>
